...
main(int argc, char *argv[]) {
     ...
     */
    {
        ...
        int count = std::count_if(vec_int.begin(), vec_int.end(),
                                  [] (int value) { return value < 1000000; });
        system_clock::time_point after = system_clock::now();
        std::cout << "Finished std::count_if " << count << " : " << ((after - before).count() / 1000000.) << std::endl;
    }
    ...
    {
        system_clock::time_point before = system_clock::now();
        int count = parallel::count_if(vec_int.begin(), vec_int.end(),
                                       [] (int value) { return value < 1000000; });
        system_clock::time_point after = system_clock::now();
        std::cout << "Finished parallel::count_if " << count << " : " << ((after - before).count() / 1000000.) << std::endl;
    }
